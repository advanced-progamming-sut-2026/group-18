...
import static com.compileordie.pvz2.models.entities.zombies.services.manager.PlantEatabilityChecker.*;
import static com.compileordie.pvz2.models.entities.zombies.services.manager.PlantVisibilityChecker.*;
        ...
        for (Zombie z : myZombies){
            for (Plant p : myPlants){
                ...
                if (z.getType()==ZombieType.EXPLORER_ZOMBIE && (Math.abs(z.getY()-p.getY())<=tileHeight/6 && Math.abs(z.getX()-p.getX())<=tileWidth*1) && isVisible(p)) {
                    if (p.isFreezedByHunter()) {p.setIsFreezedByHunter(false); p.setIsFreezedByHunterCounter(0);}
                    else {p.takeDamage((int) smashDamage);} }
                // --- پرتاب یخ توسط هانتر ---
