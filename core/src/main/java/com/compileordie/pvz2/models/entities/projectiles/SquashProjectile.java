...
public class SquashProjectile extends LobbedProjectile {
    ...
    private final double startYSq;
    private final double targetYSq;

    private double currentProgress = 0.0;
    private double jumpTimer = 0.0;
    private final double jumpDuration = 20.0; // 20 ticks = fast, aggressive jumps!

    public SquashProjectile(Plant owner,
                            double startX,
                            double startY,
                            double targetX,
                            double targetY,
                            boolean isReturning,
                            int crushesRemaining,
                            boolean isPlantFood) {
        ...
        this.startYSq = startY;
        this.targetYSq = targetY;
    ...
    public void tick(GameBoard board, double delta) {
        ...
        if (p >= 1.0) {
            ...
            if (crushesRemaining > 1) {
                ...
                if (nextTarget != null) {
                    SquashProjectile nextJump = new SquashProjectile(owner,
                        this.x,
                        this.y,
                        nextTarget.getX(),
                        nextTarget.getY(),
    ...
    private void triggerReturnOrDie(GameBoard board) {
        if (isPlantFood) {
            ...
            SquashProjectile returnJump = new SquashProjectile(owner,
                this.x,
                this.y,
                owner.getX(),
                owner.getY(),
