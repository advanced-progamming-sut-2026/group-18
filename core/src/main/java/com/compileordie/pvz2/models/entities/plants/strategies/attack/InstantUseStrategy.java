...
public class InstantUseStrategy implements AttackStrategy {
    ...
    public void attack(Plant plant, GameBoard board, int tickDelta) {
        ...
        if (name.equals("Ice-shroom")) {
            handleIceShroom(plant, board, tickDelta, name);
            return;
        }

        // --- HOT POTATO LOGIC ---
        else if (name.equals("Hot Potato")) {
            handleHotPotato(plant, board, tickDelta);
        }
    ...
    private void handleIceShroom(Plant plant, GameBoard board, int tickDelta, String name) {
    ...
    private void handleHotPotato(Plant plant, GameBoard board, int tickDelta) {
        if (plant.windupTimer == 0) {
            plant.windupTimer = 1.0;
            plant.isWindingUp = true;
    ...
    private void meltTile(GameBoard board, Tile t, int r, int c) {
        boolean meltedSomething = false;

        // Melt Ice Block obstacles
        if (t.obstacle instanceof IceBlock) {
            ((IceBlock) t.obstacle).takeDamage(9999, ProjectileType.FIRE);
        ...
        if (t.pumpkin != null && t.pumpkin.hasActiveCover()) {
            t.pumpkin.takeDamage(9999);
            meltedSomething = true;
        }
        if (t.lilyPad != null && t.lilyPad.hasActiveCover()) {
            t.lilyPad.takeDamage(9999);
            meltedSomething = true;
        }

        // Trigger the puddle visual if ice was successfully broken!
        if (meltedSomething) {
            double pX = Constants.Game.PADDING_X + (c * Constants.Game.TILE_WIDTH)
                + (Constants.Game.TILE_WIDTH / 2.0);
            double pY = Constants.Game.PADDING_Y + (r * Constants.Game.TILE_HEIGHT)
