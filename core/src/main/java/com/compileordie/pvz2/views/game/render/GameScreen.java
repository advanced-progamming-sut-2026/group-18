...
//    // 👑 حالت تست فعلا روی «فقط زامباس دارک» تنظیم شده (طبق درخواست). برای
//    // برگشتن به تست‌اسپاون همه‌ی انواع زامبی یا تست زامباس مصر، فقط
//    // ZombieTestSpawner.TestMode رو با یکی از سه مقدار NORMAL/EGYPT_ZOMBOSS/
//    // DARK_ZOMBOSS عوض کن.
//    private final ZombieTestSpawner testSpawner = new ZombieTestSpawner(ZombieTestSpawner.TestMode.DARK_ZOMBOSS);
//
...
//
//        // 💥 مخصوص زامباس: تایمر انیمیشن افکت انفجار موشک (ZOMBOSS_MISSILE_EXPLOSION)
//        // که در طول حالت boom پخش می‌شه؛ وقتی یک boom جدید شروع بشه (گذر از false
//        // به true) از صفر ریست می‌شه تا لوپ از فریم اول شروع بشه.
...
//
//        if (sunAmountLabel != null && AppModel.gameSession != null) {
//            int amount = AppModel.gameSession.gameBoard.economyManager.sunAmount;
//            sunAmountLabel.setText(String.format("%04d", amount)); // همیشه ۴ رقمی نشون میده مثل 0050
//        }
//
//        batch.begin();
//
...
//        drawTombs(delta);
//        drawFireTiles(delta); // خونه‌های در حال سوختن (isOnFire) - در کل بازی، مستقل از زامباس
//        drawDeadZombies(delta);
//        drawZombies(delta);
//        drawZombossExplosion(delta);
//        drawDarkZombossLaserSquare(delta);
...
//
//        // 👑 نوار سلامتی زامباس - عمدا بعد از بازگردوندن ماتریس (تا لرزش دوربین
//        // روش تاثیر نذاره، چون این یه عنصر HUD ثابت گوشه‌ی صفحه‌ست، نه چیزی که
//        // با دنیای بازی حرکت کنه).
//        drawZombossHealthBar(delta);
//
//        batch.end();
//
...
//            return new Color(0.25f, 0.55f, 0.85f, 0.85f); // آبی روشن و شفاف
//        }
...
//        }
//
//        // 🤿 غواص زیر آب (هم UNDERWATER_SURFACED هم UNDERWATER_NOT_SURFACED):
//        // بدن باید «شیشه‌ای» آبی پررنگ بشه - یعنی هنوز پیداست (شفافیت کامل نه)
//        // ولی خیلی به آبیِ اشباع‌شده می‌زنه. طبق درخواست فقط ۳/۴ پایینیِ بدن
//        // (بدون سر) باید این‌جوری بشه؛ چون drawPart برای رسم مجزای سر با
//        // اسکیل/فلیپ درست هیچ پارامتری نداره (احتمال میس‌الاین‌منت بالاست)، فعلا
//        // امن‌ترین و قابل‌اعتمادترین نسخه پیاده شده: کل بدن (شامل سر هم) آبی
//        // می‌شه، نه فقط ۳/۴ پایینی. اگه بعد از دیدن نتیجه هنوز اصرار به حذف
//        // دقیق سر داشتید، بگید تا نسخه‌ی drawPart (با ریسک میس‌الاین‌منت) رو
//        // هم اضافه کنم.
...
//                return new Color(0.0f, 0.85f, 1.0f, 1f); // آبی آسمانی/فیروزه‌ای پررنگ و درخشان، نیمه‌شفاف (شیشه‌ای) - عمدا از CHILLED فاصله گرفته شده (G بالاتر، B اشباع کامل)
//            }
//        }
//
...
//        }
//
//        // 🌪️ اگه این زامبی از وسط زمین اسپاون شده (مثلا زامبی‌ای که Tombraiser از
//        // قبر بیرون کشیده)، اول باید فقط افکت SANDSTORM_TOP پخش بشه؛ خودِ زامبی
//        // اصلا رندر نمی‌شه تا این افکت تموم بشه. دقیقا هم‌الگو با isFlyingIn پایین.
//        if (zombie.isSandstormSpawning()) {
//            drawSandstormSpawningZombie(zombie, state, delta);
//            return;
//        }
//
...
//
//        // ==========================================
//        // 👑 زامباس (EgyptZomboss) - حالت‌های اختصاصی خودش، کاملا مستقل از منطق
//        // عمومی eat/walk بقیه‌ی زامبی‌ها. اولویت‌بندی (از دراماتیک‌ترین به
//        // ساده‌ترین): boom > stun > spawnZombies > حرکت (بر اساس علامت سرعت) >
//        // idle. توجه: مدل خودش "smash" (حمله‌ی نزدیک) رو با ست‌کردن xSpeed مثبت
//        // پیاده می‌کنه، پس نیازی به یه انیمیشن جدا براش نیست - همون
//        // "walk_forward" طبیعی پوشششش می‌ده.
//        // 🚩 اولویت boom>stun>spawn دقیقا حدسی/منطقیه، نه چیزی که صراحتا گفته
//        // باشید؛ اگه ترتیب دیگه‌ای مدنظرتونه بگید.
//        // ==========================================
//        if (zombie.getType() == ZombieType.ZOMBOSS_IN_EGYPT) {
//            EgyptZomboss zomboss = (EgyptZomboss) zombie;
//            if (zomboss.boom) {
//                animName = "rocket_launch";
//            } else if (zomboss.stun) {
//                animName = "stun_loop";
//            } else if (zomboss.spawnZombies) {
//                animName = "zombie_portal_loop";
//            } else if (zombie.getXSpeed() > 0) {
...
//        }
//        // ==========================================
//        // 👑 زامباس دارک (DarkZomboss) - نگاشت مستقیم فلگ‌های مدل به انیمیشن،
//        // دقیقا طبق ترتیب اولویتی که خودِ DarkZomboss.tick() هم رعایت می‌کنه
//        // (stun > spawnZombies > smash > boom > idle).
//        // ==========================================
//        else if (zombie.getType() == ZombieType.ZOMBOSS_IN_DARK) {
//            DarkZomboss darkZomboss = (DarkZomboss) zombie;
//            if (darkZomboss.stun) {
//                animName = "stun_loop";
//            } else if (darkZomboss.spawnZombies) {
//                animName = "vulnerable_loop";
//            } else if (darkZomboss.smash) {
//                animName = "fire_attack_idle";
//            } else if (darkZomboss.boom) {
//                animName = "fire_bomb_loop";
//            } else {
//                animName = "idle";
//            }
...
//
//                    batch.setTransformMatrix(scaledTransform);
//                    player.draw(batch, resolvedPath, animName, renderAnimTime, drawX, drawY, state.flip, visibilityMap);
//                    batch.setTransformMatrix(originalTransform);
//                } else {
//                    player.draw(batch, resolvedPath, animName, renderAnimTime, drawX, drawY, scaleX, scaleY, animName.equals("cannon_fire")?false:state.flip);
//                }
//
//                if (oldColor != null) {
//                    batch.setColor(oldColor);
...
//    }
//
//    // مسیر افکت طوفان شنی که موقع اسپاون از وسط زمین (نه از لبه‌ی چپ عادی) پخش
//    // می‌شه - مثلا زامبی‌ای که Tombraiser از قبر بیرون می‌کشه.
//    private static final String SANDSTORM_SPAWN_PAM = "768/INITIAL/EFFECTS/SANDSTORM_TOP/SANDSTORM_TOP.PAM";
//    // 🚩 اسم کلیپ داخل این PAM حدسیه (مطمئن نیستم دقیقا "animation" باشه) -
//    // اگه لود/رندرش خطا داد یا اسمش عوض بود، همین رشته رو با اسم واقعی
//    // (که از pvz-asset-browser یا player.clips(SANDSTORM_SPAWN_PAM) می‌تونید
//    // پیدا کنید) جایگزین کنید.
//    private static final String SANDSTORM_SPAWN_CLIP = "loop";
//
//    /**
//     * در طول SANDSTORM_DURATION ثانیه‌ی اول اسپاون یک زامبی از وسط زمین، به‌جای
//     * خودِ زامبی، فقط این افکت (به‌صورت loop) دقیقا سرجای اسپاونش پخش می‌شه.
//     * بعد از تمام‌شدنش (Zombie.tick() خودش sandstormRemaining رو صفر می‌کنه)،
//     * این متد دیگه صدا زده نمی‌شه و drawSingleZombie طبق روال عادی زامبی رو
//     * نشون می‌ده - دقیقا هم‌الگو با drawFlyingInZombie پایین‌تر.
//     */
//    private void drawSandstormSpawningZombie(Zombie zombie, ZombieRenderState state, float delta) {
//        // =====================================================================
//        // 🔍 تشخیص فوری و بلند (نه ساکت!): قبل از هر تلاشی برای رندر، خودمون
//        // مستقیم چک می‌کنیم فایل PAM واقعا روی دیسک هست یا نه - دقیقا با همون
//        // تکنیک existsOnDisk() که ZombieVisualRegistry استفاده می‌کنه
//        // (Gdx.files.internal("pvz-assets/IMAGES/" + مسیر).exists()).
//        //
//        // چرا این چک لازمه و کافی نیست فقط منتظر خطا از player.draw(...) بمونیم؟
//        // چون PamPlayer.draw(...) وقتی فایل asset لود نشه، هیچ Exception ای پرت
//        // نمی‌کنه! لودِ واقعی PAM تو یه Thread پس‌زمینه (PamPlayerWorker) انجام
//        // می‌شه و اگه اونجا خطا بخوره، فقط e.printStackTrace() می‌شه (نه گدکس‌لاگ،
//        // نه پرتاب به Thread اصلی) - یعنی player.draw(...) رو صدا زدن، هیچ اتفاقی
//        // نمی‌افته (نه کرش، نه لاگ)، فقط ساکت هیچی رسم نمی‌شه. دقیقا همینه که
//        // باعث شده تا الان بدون هیچ خطای قابل مشاهده‌ای، گردباد پلی نشه.
//        // =====================================================================
//        if (brokenAssets.contains(SANDSTORM_SPAWN_PAM)) {
//            return; // قبلا لاگ زدیم؛ فقط ساکت رد می‌شیم که هر فریم لاگ تکراری نریزه
//        }
//
...
//                    + "این یعنی PamPlayer هیچ‌وقت نمی‌تونه لودش کنه (نه کرش می‌کنه نه\n"
//                    + "خطای قابل‌مشاهده می‌ده - فقط ساکت هیچی رسم نمی‌کنه)، برای همینه که\n"
//                    + "الان اصلا گردباد دیده نمی‌شه.\n"
//                    + "راه‌حل: اسم/مسیر واقعی فایل PAM این افکت رو (زیر پوشه‌ی\n"
//                    + "pvz-assets/IMAGES/768/INITIAL/EFFECTS/...) پیدا کن و مقدار ثابت\n"
//                    + "SANDSTORM_SPAWN_PAM رو تو GameScreen.java با همون جایگزین کن.\n"
//                    + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
//            return; // اصلا سعی نکن رسم کنی؛ همون‌جوری ساکت هیچی نشون نمی‌داد
//        }
//
//        state.animTime += delta; // مستقل و پیوسته، وابسته به eat/walk نیست
//
...
//                batch,
//                SANDSTORM_SPAWN_PAM,
//                SANDSTORM_SPAWN_CLIP,
//                state.animTime,
//                baseX,
//                baseY,
//                0.6f,
//                0.6f,
//                true // loop=true طبق درخواست
//            );
//        } catch (Throwable e) {
//            brokenAssets.add(SANDSTORM_SPAWN_PAM);
//            Gdx.app.error("PVZ-ASSET-MISSING",
//                "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
//                    + "❌ رندر افکت SANDSTORM_TOP ناموفق بود.\n"
//                    + "مسیر: " + SANDSTORM_SPAWN_PAM + " | کلیپ: " + SANDSTORM_SPAWN_CLIP + "\n"
//                    + "اگه اسم کلیپ اشتباهه، SANDSTORM_SPAWN_CLIP رو تو GameScreen.java عوض کن.\n"
//                    + "دلیل دقیق خطا: " + e + "\n"
//                    + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", e);
//        }
//    }
//
//    // مسیر و کلیپ افکت انفجار موشک زامباس (فقط در طول حالت boom پخش می‌شه)
...
//
//    /**
//     * موقع boom بودن زامباس، توی خونه‌ای که zomboss.r / zomboss.c مشخص می‌کنه،
//     * افکت انفجار موشک رو به‌صورت loop پخش می‌کنه. این متد مستقل از
//     * drawSingleZombie صدا زده می‌شه (طبق درخواست: خودِ زامباس هم جدا رندر
//     * می‌شه، هم این افکت روی خونه‌ی هدف جدا رندر می‌شه).
//     *
//     * فرمول مختصات (طبق درخواست، هم‌الگو با Tomb): برای Y از سطر (r) و برای X
//     * از ستون (c) استفاده می‌شه، هرکدوم با پدینگ متناظر خودشون:
//     *   y = (r * TILE_HEIGHT + PADDING_Y_REALITY) * METER_TO_PIX
//     *   x = (c * TILE_WIDTH  + PADDING_X_REALITY) * METER_TO_PIX
//     * 🚩 این تفسیر من از فرمول توضیح‌داده‌شده‌ست (برای X با قیاس روی فرمول Y که
//     * صریح گفته بودید) - اگه دقیقا منظورتون همین نبود، فقط همین دو خط رو عوض کن.
//     */
//    private void drawZombossExplosion(float delta) {
//        if (AppModel.gameSession == null || player == null) {
//            return;
//        }
//        if (brokenAssets.contains(ZOMBOSS_EXPLOSION_PAM)) {
//            return;
//        }
//
...
//                continue; // فقط دقیقا در طول boom پخش می‌شه
//            }
//            if (!state.wasBooming) {
//                state.bombAnimTime = 0f; // شروع یه boom جدید -> لوپ از فریم اول
//                state.wasBooming = true;
//            }
//            state.bombAnimTime += delta;
//
//            if (zomboss.r < 0 || zomboss.c < 0) {
//                continue; // هنوز خونه‌ی هدف مشخص نشده
//            }
//
...
//                player.draw(
//                    batch,
//                    ZOMBOSS_EXPLOSION_PAM,
//                    ZOMBOSS_EXPLOSION_CLIP,
//                    state.bombAnimTime,
//                    explosionX,
//                    explosionY,
//                    0.6f,
...
//                Gdx.app.error("PVZ-ASSET-MISSING",
//                    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
//                        + "❌ رندر افکت انفجار زامباس ناموفق بود.\n"
//                        + "مسیر: " + ZOMBOSS_EXPLOSION_PAM + " | کلیپ: " + ZOMBOSS_EXPLOSION_CLIP + "\n"
//                        + "دلیل دقیق خطا: " + e + "\n"
//                        + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", e);
//            }
//        }
...
//
//    // ==========================================================================
//    // 👑 زامباس دارک - افکت لیزر ۲ خونه‌ای موقع boom
//    // ==========================================================================
//
...
//
//    /**
//     * موقع boom بودن زامباس دارک، دقیقا مثل زامباس مصر، ولی این‌بار روی *دو*
//     * خونه‌ی هدف (r1,c1) و (r2,c2) - نه یکی - افکت لیزر رو پخش می‌کنه.
//     * طبق درخواست: clip="impact" با loop=false (برخلاف افکت انفجار مصر که
//     * loop=true بود).
//     * فرمول مختصات دقیقا همونیه که برای افکت انفجار مصر هم استفاده شده.
//     */
//    private void drawDarkZombossLaserSquare(float delta) {
//        if (AppModel.gameSession == null || player == null) {
//            return;
//        }
//        if (brokenAssets.contains(ZOMBOSS_DARK_LASER_PAM)) {
//            return;
//        }
//
//        for (Zombie zombie : AppModel.gameSession.gameBoard.getAllZombies()) {
//            if (zombie.getType() != ZombieType.ZOMBOSS_IN_DARK) {
//                continue;
//            }
...
//            }
//            if (!state.wasBooming) {
//                state.bombAnimTime = 0f; // شروع یه boom جدید -> از فریم اول
//                state.wasBooming = true;
//            }
//            state.bombAnimTime += delta;
//
//            drawDarkZombossLaserTile(zomboss.r1, zomboss.c1, state.bombAnimTime);
//            drawDarkZombossLaserTile(zomboss.r2, zomboss.c2, state.bombAnimTime);
//        }
//    }
//
//    private void drawDarkZombossLaserTile(int r, int c, float animTime) {
//        if (r < 0 || c < 0) {
//            return; // هنوز خونه‌ی هدف مشخص نشده
//        }
//
...
//            player.draw(
//                batch,
//                ZOMBOSS_DARK_LASER_PAM,
//                ZOMBOSS_DARK_LASER_CLIP,
//                animTime,
//                explosionX,
//                explosionY,
//                0.6f,
//                0.6f,
//                true // loop=false طبق درخواست
//            );
//        } catch (Throwable e) {
//            brokenAssets.add(ZOMBOSS_DARK_LASER_PAM);
//            Gdx.app.error("PVZ-ASSET-MISSING",
//                "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
//                    + "❌ رندر افکت لیزر زامباس دارک ناموفق بود.\n"
//                    + "مسیر: " + ZOMBOSS_DARK_LASER_PAM + " | کلیپ: " + ZOMBOSS_DARK_LASER_CLIP + "\n"
//                    + "دلیل دقیق خطا: " + e + "\n"
//                    + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", e);
//        }
//    }
//
//    // نسبت طول به عرض نوار = ۱۰ طبق درخواست. عرض رو خودم یه عدد معقول گرفتم؛
//    // اگه اندازه‌ی دیگه‌ای مدنظرتونه همین یکی رو عوض کن، بقیه (ارتفاع/تقسیم‌بندی)
//    // خودکار هم‌نسبت باهاش تنظیم می‌شن.
//    private static final float ZOMBOSS_BAR_WIDTH_PX = 400f;
//    private static final float ZOMBOSS_BAR_ASPECT_RATIO = 10f; // طول:عرض = ۱۰
//    private static final float ZOMBOSS_BAR_HEIGHT_PX = ZOMBOSS_BAR_WIDTH_PX / ZOMBOSS_BAR_ASPECT_RATIO;
//    private static final float ZOMBOSS_BAR_MARGIN_PX = 24f; // فاصله از لبه‌ی پایین/راست صفحه
//    private static final float ZOMBOSS_BAR_DIVIDER_WIDTH_PX = 3f; // ضخامت خط‌های زرد ۱/۳ و ۲/۳
//
//    // قرمز پررنگ (بخش سلامتی باقی‌مونده - سمت راستِ کله) در برابر قرمز کمرنگ
//    // (بخشی که کله ازش رد شده - یعنی همون مقدار جونی که از دست رفته)
...
//
//    private static final String ZOMBOSS_HEAD_PAM = "768/FULL/EFFECTS/PRIZE_PINATA_ZOMBOSS/PRIZE_PINATA_ZOMBOSS.PAM";
//    private static final String ZOMBOSS_HEAD_CLIP = "idle";
//    // ارتفاع هدفِ رندر کله (پیکسل) - چون این یه آیکون HUD کوچیکه نه یه اسپرایت
//    // تو دنیای بازی، مقیاسش رو با bounds() واقعی PAM حساب می‌کنیم تا کش/کوتاه
//    // نشه (دقیقا همون تکنیکی که برای تصویر مه‌ی پنل پاز استفاده شده).
//    private static final float ZOMBOSS_HEAD_TARGET_HEIGHT_PX = ZOMBOSS_BAR_HEIGHT_PX * 3.2f;
//
//    private float zombossHeadAnimTime = 0f;
//    private Texture zombossBarSolidTexture; // یه تکسچر ۱x۱ سفید، برای رسم مستطیل‌های رنگی نوار
//
//    /**
//     * جونِ فعلیِ زامباس رو برمی‌گردونه، بدون اهمیت اینکه EgyptZomboss باشه یا
//     * DarkZomboss - هرکدوم فیلد public "health" مخصوص خودشون رو دارن (نه از
//     * طریق کلاس پایه‌ی مشترک)، پس این‌جا با instanceof هردو رو پوشش می‌دیم.
//     */
//    private double getZombossCurrentHealth(Zombie zombie) {
//        if (zombie instanceof EgyptZomboss egyptZomboss) {
//            return egyptZomboss.health;
//        }
//        if (zombie instanceof DarkZomboss darkZomboss) {
//            return darkZomboss.health;
//        }
//        return zombie.getMaxHealth(); // بی‌خطر: یعنی نوار کامل نشون بده، نه کرش
//    }
//
//    /**
//     * اول بررسی می‌کنه آیا اصلا زامباسی روی زمین هست (بر اساس اینکه
//     * zombie.getType().name() شامل رشته‌ی "ZOMBOSS" باشه یا نه). اگه هیچ
//     * زامباسی نبود، کلا هیچی رسم نمی‌شه.
//     *
//     * 🛡️ چون حالا هم EgyptZomboss هم DarkZomboss ممکنه روی زمین باشن (هردو
//     * getType().name() شامل "ZOMBOSS" هستن) و هرکدوم جداگانه یه فیلد public
//     * "health" مخصوص خودشون دارن (نه از طریق یه اینترفیس/کلاس پایه‌ی مشترک)،
//     * دیگه نمی‌شه بدون چک نوع مستقیم به EgyptZomboss کست کرد - این کار برای
//     * DarkZomboss یه ClassCastException می‌ده. برای همین این‌جا از
//     * getZombossCurrentHealth(Zombie) استفاده می‌کنیم که با instanceof هردو
//     * نوع رو (بدون دست‌زدن به خودِ EgyptZomboss.java) پشتیبانی می‌کنه.
//     */
//    private void drawZombossHealthBar(float delta) {
//        if (AppModel.gameSession == null || player == null) {
//            return;
//        }
//
...
//        }
//        if (zomboss == null) {
//            return; // هیچ زامباسی تو زمین نیست -> هیچی رسم نمی‌شه
//        }
//
//        if (zombossBarSolidTexture == null) {
//            zombossBarSolidTexture = createSolidTexture(Color.WHITE);
//        }
//
...
//
//        // 🎯 پیشرفت: ۰ = جون کامل (کله سمت چپ نواره)، ۱ = جون صفر (کله سمت راست نواره)
//        double maxHealth = zomboss.getMaxHealth() > 0 ? zomboss.getMaxHealth() : 1.0;
//        float healthFraction = (float) Math.max(0.0, Math.min(1.0, getZombossCurrentHealth(zomboss) / maxHealth));
//        float progress = 1f - healthFraction;
//        float headX = barX + progress * ZOMBOSS_BAR_WIDTH_PX;
//
//        Color prevColor = batch.getColor().cpy();
//
//        // ۱) بخش سمت چپِ کله (جونی که از دست رفته) - قرمز کمرنگ
//        if (headX > barX) {
//            batch.setColor(ZOMBOSS_BAR_FADED_RED);
//            batch.draw(zombossBarSolidTexture, barX, barY, headX - barX, ZOMBOSS_BAR_HEIGHT_PX);
//        }
//        // ۲) بخش سمت راستِ کله (جون باقی‌مونده) - قرمز پررنگ
//        if (headX < barX + ZOMBOSS_BAR_WIDTH_PX) {
//            batch.setColor(ZOMBOSS_BAR_VIVID_RED);
//            batch.draw(zombossBarSolidTexture, headX, barY, (barX + ZOMBOSS_BAR_WIDTH_PX) - headX, ZOMBOSS_BAR_HEIGHT_PX);
//        }
//
//        // ۳) دو خط زرد ثابت روی ۱/۳ و ۲/۳ عرض نوار (تقسیم به سه قسمت مساوی) -
//        // این‌ها با حرکت کله جابه‌جا نمی‌شن، همیشه سرجای ثابت خودشون هستن.
...
//
//        // ۴) کله‌ی زامباس (PRIZE_PINATA_ZOMBOSS، حالت idle) - دقیقا روی همون
//        // نقطه‌ای که نوار رو به دو قسمت کمرنگ/پررنگ تقسیم می‌کنه.
//        if (!brokenAssets.contains(ZOMBOSS_HEAD_PAM)) {
//            try {
...
//                Gdx.app.error("PVZ-ASSET-MISSING",
//                    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
//                        + "❌ رندر کله‌ی زامباس (نوار سلامتی) ناموفق بود.\n"
//                        + "مسیر: " + ZOMBOSS_HEAD_PAM + " | کلیپ: " + ZOMBOSS_HEAD_CLIP + "\n"
//                        + "دلیل دقیق خطا: " + e + "\n"
//                        + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", e);
//            }
//        }
...
//
//    // ==========================================================================
//    // 🔥 خونه‌های در حال سوختن (Tile.isOnFire) - در کل بازی، مستقل از هرگونه
//    // زامباس یا چپتر خاص؛ هر خونه‌ای که isOnFire=true باشه این افکت روش پخش می‌شه.
//    // ==========================================================================
//
//    private static final String FIRE_TILE_PAM = "768/INITIAL/EFFECTS/FIREPEASHOOTER_FIRE/FIREPEASHOOTER_FIRE.PAM";
//    private static final String FIRE_TILE_CLIP = "idle2";
//
//    private final Map<Tile, Float> fireTileAnimTimes = new HashMap<>();
//
//    private void drawFireTiles(float delta) {
//        if (AppModel.gameSession == null || player == null) {
//            return;
//        }
//        if (brokenAssets.contains(FIRE_TILE_PAM)) {
//            return;
//        }
//
...
//
//                float animTime = fireTileAnimTimes.getOrDefault(tile, 0f) + delta;
//                fireTileAnimTimes.put(tile, animTime);
//
...
//                    Gdx.app.error("PVZ-ASSET-MISSING",
//                        "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
//                            + "❌ رندر افکت آتیشِ خونه ناموفق بود.\n"
//                            + "مسیر: " + FIRE_TILE_PAM + " | کلیپ: " + FIRE_TILE_CLIP + "\n"
//                            + "دلیل دقیق خطا: " + e + "\n"
//                            + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", e);
//                }
//            }
//        }
//
//        // پاک‌سازی وضعیت خونه‌هایی که دیگه آتیش نیستن (وگرنه Map بی‌نهایت بزرگ می‌شه)
//        fireTileAnimTimes.keySet().removeIf(tile -> !stillOnFire.contains(tile));
//    }
//
//    /**
...
//
//    // وضعیت رندر مخصوص هر خورشید (برای انیمیشن و تایمر محو شدن)
//    private static final class SunRenderState {
//        float animTime = 0f;
//        boolean isFading = false;
//        float fadeTimer = 0f; // از ۰ تا ۰.۵ ثانیه
//    }
//
//    // ممپ نگه‌داری وضعیت خورشیدها
//    private final Map<Object, SunRenderState> sunRenderStates = new HashMap<>();
//
//    // لیبل نمایش تعداد خورشیدها
//    private Label sunAmountLabel;
//
//    private void setupGameUI() {
//        // یک Table تمام‌صفحه که محتواش رو می‌چسبونه به بالا سمت چپ
//        Table topUI = new Table();
//        topUI.setFillParent(true);
//        topUI.top().left();
//
//        // Table داخلی برای کنار هم گذاشتن آیکون و عدد
//        Table sunBox = new Table();
//
//        // لود کردن آیکون خورشید
//        TextureRegion sunRegion = textureBank.region("IMAGE_UI_SEASONS_UNCOMPRESSED_PVZ2_SEASONS_UIASSET_ICON_SUN");
//        if (sunRegion != null) {
//            sunBox.add(new Image(sunRegion)).size(64, 64).padRight(10); // سایز آیکون رو می‌تونی تنظیم کنی
//        }
//
//        // ساخت لیبل ۴ رقمی با فونت pvz (مثلا استایل medium_outline)
//        sunAmountLabel = new Label("0000", skin, "medium_outline");
//        sunAmountLabel.setFontScale(1.2f);
//        sunBox.add(sunAmountLabel);
//
//        // اضافه کردن کل باکس به گوشه صفحه با کمی فاصله (Padding)
//        topUI.add(sunBox).pad(20);
//
//        // اضافه کردن به Stage اصلی UI
//        uiStage.addActor(topUI);
//    }
//
//    private void drawSuns(float delta) {
//        if (AppModel.gameSession == null || player == null) return;
//
//        // ۱. گرفتن مختصات دقیق موس/لمس در دنیای بازی (مستقل از کلیک شدن)
//        Vector3 mousePos = new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0);
//        viewport.unproject(mousePos);
//
//        // ۲. گرفتن لیست خورشیدها از EconomyManager
...
//        while (iterator.hasNext()) {
//            var sun = iterator.next(); // اینجا از آبجکت مدل خورشید خودت استفاده می‌کنه
//
//            SunRenderState state = sunRenderStates.computeIfAbsent(sun, s -> new SunRenderState());
//            state.animTime += delta;
//
//            // تبدیل موقعیت متری مدل به پیکسلی رندر
//            float baseX = (float) sun.getX() * Constants.UI.METER_TO_PIX;
//            float baseY = (float) sun.getY() * Constants.UI.METER_TO_PIX;
//
//            // ۳. بررسی ورود موس به محوطه (شعاع ۵۰ پیکسل -> توان دومش می‌شه ۲۵۰۰)
//            if (!state.isFading) {
...
//                    // الف) اضافه‌شدن مقدار خورشید به سیستم اقتصاد
//                    // (فرض کردم متد getAmount داری، اگه ثابته مستقیم عدد بده)
//                    AppModel.gameSession.gameBoard.economyManager.sunAmount += sun.type.value;
//
//                    // ب) حذف امن خورشید از لیست و استیت‌ها
//                    iterator.remove();
//                    sunRenderStates.remove(sun);
//                    continue; // از رندر این فریم رد می‌شیم چون محو شده
//                }
//            }
//
//            // ۵. تنظیم مقیاس و انیمیشن بر اساس Type
//            String pamPath = SUN_PAM;
//            float scale = 1.0f; // نرمال، مدیوم، لارج هم‌اندازه
//
//            // (اگه توی مدلت enum هست یا متد دیگه داره، شرط‌ها رو مطابقش تغییر بده)
//            String typeKey = sun.type.name();
//
//            if (typeKey.contains("RADIOACTIVE")) {
//                pamPath = RADIOACTIVE_SUN_PAM;
//            } else if (typeKey.contains("SPECIAL")) {
//                scale = 1.5f; // یک و نیم برابر بزرگتر
//            }
//
...
//            Color oldColor = batch.getColor().cpy();
//            try {
//                // اعمال شفافیت (آلفا) برای افکت محو شدن
...
//                    scale*0.8f,
//                    scale*0.8f,
//                    true // خورشید نیازی به فلیپ چپ و راست نداره
//                );
//            } catch (Throwable e) {
//                brokenAssets.add(pamPath);
//                Gdx.app.error("PVZ-SUN", "❌ رندر خورشید خطا داد: " + e.getMessage());
//            } finally {
//                batch.setColor(oldColor); // بازگردانی رنگ برای رندرهای بعدی
//            }
//        }
//    }
//}
