...
public enum Command {
    ...
    // Game Session Menu
    ADVANCE_TIME("advance\\s+time\\s+-t\\s+(?<count>\\S+)\\s+ticks"),
    COLLECT_SUN("collect\\s+sun\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
    SHOW_SUN_AMOUNT("show\\s+sun\\s+amount"),
    CHEAT_ADD_SUN("cheat\\s+add\\s+-n\\s+(?<count>\\S+)\\s+suns"),
    RELEASE_THE_NUKE("release\\s+the\\s+nuke"),
    PLANT_PLANT("plant\\s+plant\\s+-t\\s+(?<type>.+)\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
    CHEAT_REMOVE_COOLDOWN("cheat\\s+remove-cooldown"),
    PLUCK_PLANT("pluck\\s+plant\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
    FEED_PLANT("feed\\s+plant\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
    CHEAT_ADD_PLANT_FOOD("cheat\\s+add-plant-food"),
    SHOW_MAP("show\\s+map"),
    SHOW_PLANTS_STATUS("show\\s+plants\\s+status"),
    SHOW_TILES_STATUS("show\\s+tile\\s+status\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
    START_ZOMBIE_WAVES("start\\s+zombie\\s+waves"),
    ZOMBIE_INFO("zombies\\s+info"),
    SPAWN_ZOMBIE("cheat\\s+spawn-zombie\\s+-t\\s+(?<type>\\S+)\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
    BREAK_VASE("break\\s+vase\\s+-l\\s+\\((?<x>\\S+),\\s+(?<y>\\S+)\\)"),
